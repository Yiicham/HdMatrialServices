...
{
    public partial class frmMaritalPlanningList : XtraForm
    {
        public string _Martial = null;
        public string _Contains = null;
        public frmMaritalPlanningList(string Martial)
        {
            _Martial = Martial;
            if (_Martial == "主材入库")
                _Contains = "ProfilePlan";
            else if (_Martial == "板材入库")
                _Contains = "PlatePlan";
            else if (_Martial == "附件入库")
                _Contains = "PartsPlan";
            InitializeComponent();
        }

        private void frmMaritalPlanningList_Load(object sender, EventArgs e)
        {
            this.Text = _Martial;
        ...
        private void btAddNew_ItemClick(object sender, DevExpress.XtraBars.ItemClickEventArgs e)
        {
            ...
            if (Properties.Settings.Default.VerID == 1)
                ...
                "Province","City","County", "SheetNumber"  ,"Originator", "SheetDate" ,"CalculateTotal"};
            using (frmAddPlanning myForm = new frmAddPlanning(null, hideNames, _Martial, 1))
            {
                if (myForm.ShowDialog() == DialogResult.OK)
                    Reflesh();
            }
            ...
            try
            {
                ...
                using (ChannelFactory<IhdSQLite> channelFactory = new ChannelFactory<IhdSQLite>("hdDbClient"))
                {
                    ...
                    using (OperationContextScope loginScope = new OperationContextScope(myFile as IClientChannel))
                    {
                        ...
                        if(_Martial == "主材入库")
                        {
                            var dtList = ModelConvertHelper<ProfilePlan>.ConvertToModel(dt);
                            BindingList<ProfilePlan> bindList = new BindingList<ProfilePlan>(dtList);
                        ...
                        {
                            var dtList = ModelConvertHelper<PlatePlan>.ConvertToModel(dt);
                            BindingList<PlatePlan> bindList = new BindingList<PlatePlan>(dtList);
                            gridControl2.DataSource = bindList;
                            //求合列
                            gridView2.OptionsView.ShowFooter = true;
                            gridView2.Columns["Number"].SummaryItem.SummaryType = DevExpress.Data.SummaryItemType.Sum;
                        ...
                        {
                            var dtList = ModelConvertHelper<PartsPlan>.ConvertToModel(dt);
                            BindingList<PartsPlan> bindList = new BindingList<PartsPlan>(dtList);
                            gridControl2.DataSource = bindList;
                            //求合列
                            gridView2.OptionsView.ShowFooter = true;
                            gridView2.Columns["Number"].SummaryItem.SummaryType = DevExpress.Data.SummaryItemType.Sum;
        ...
        private void bbtClearRow_ItemClick(object sender, DevExpress.XtraBars.ItemClickEventArgs e)
        {
            ...
            try
            {
                if (_Martial == "主材入库")
                {
                    BindingList<ProfilePlan> xcList = (BindingList<ProfilePlan>)gridControl2.DataSource;
                    xcList.Clear();
                }
                else if (_Martial == "板材入库")
                {
                    BindingList<PlatePlan> xcList = (BindingList<PlatePlan>)gridControl2.DataSource;
                    xcList.Clear();
                }
                else if (_Martial == "附件入库")
                {
                    BindingList<PartsPlan> xcList = (BindingList<PartsPlan>)gridControl2.DataSource;
                    xcList.Clear();
                }
            }
            catch (Exception ex)
                ...
                using (ChannelFactory<IhdSQLite> channelFactory = new ChannelFactory<IhdSQLite>("hdDbClient"))
                {
                    ...
                    using (OperationContextScope loginScope = new OperationContextScope(myFile as IClientChannel))
                    {
                        ...
                        if (_Martial == "主材入库")
                        {
                            ...
                            //添加修改
                            foreach (ProfilePlan xc in xcList)
                            {
                            ...
                            //移除
                            foreach (KeyValuePair<long, ProfilePlan> xc in dtDictionary)
                            {
                                bool hasDelete = true;
                                foreach (ProfilePlan txc in xcList)
                                {
                                ...
                                if (hasDelete)
                                {
                                    myFile.ExecuteNonQuery(HDModel.dbVerID, "DELETE FROM ProfilePlan WHERE ID=" + xc.Value.ID);
                                }
                            }
                            //同时要更新单据的修改时间
                        ...
                        else if (_Martial == "板材入库")
                        {
                            ...
                            var dtDictionary = dtList.ToDictionary(m => m.ID);
                            //添加修改
                            foreach (PlatePlan xc in xcList)
                            {
                                xc.SheetName = dr.SheetNumber;
                                if (xc.ID > 0)
                                {
                            ...
                            //移除
                            foreach (KeyValuePair<long, PlatePlan> xc in dtDictionary)
                            {
                                bool hasDelete = true;
                                foreach (PlatePlan txc in xcList)
                                {
                                    if (txc.ID > 0 && txc.ID == xc.Value.ID)
                                    {
                                        hasDelete = false;
                                ...
                                if (hasDelete)
                                {
                                    myFile.ExecuteNonQuery(HDModel.dbVerID, "DELETE FROM PlatePlan WHERE ID=" + xc.Value.ID);
                                }
                            }
                            //同时要更新单据的修改时间
                            dr.SheetDate = System.DateTime.Now;
                        ...
                        else if (_Martial == "附件入库")
                        {
                            ...
                            var dtDictionary = dtList.ToDictionary(m => m.ID);
                            //添加修改
                            foreach (PartsPlan xc in xcList)
                            {
                                xc.SheetName = dr.SheetNumber;
                                if (xc.ID > 0)
                                {
                            ...
                            //移除
                            foreach (KeyValuePair<long, PartsPlan> xc in dtDictionary)
                            {
                                bool hasDelete = true;
                                foreach (PartsPlan txc in xcList)
                                {
                                    if (txc.ID > 0 && txc.ID == xc.Value.ID)
                                    {
                                        hasDelete = false;
                                ...
                                if (hasDelete)
                                {
                                    myFile.ExecuteNonQuery(HDModel.dbVerID, "DELETE FROM PartsPlan WHERE ID=" + xc.Value.ID);
                                }
                            }
                            //同时要更新单据的修改时间
                            dr.SheetDate = System.DateTime.Now;
        ...
        private void bbtImport_ItemClick(object sender, DevExpress.XtraBars.ItemClickEventArgs e)
        {
            ...
            try
            {
                if (_Martial == "主材入库")
                {
                    using (frmImport myForm = new frmImport(ImportMode.IProfilePlan, hiddenNames))
                    {
                        if (myForm.ShowDialog() == DialogResult.OK)
                        {
                            BindingList<ProfilePlan> prdList = (BindingList<ProfilePlan>)myForm.objLists;
                            BindingList<ProfilePlan> xcList = (BindingList<ProfilePlan>)gridControl2.DataSource;
                            foreach (ProfilePlan prd in prdList)
                                xcList.Add(prd);
                        }
                    }
                ...
                {
                    using (frmImport myForm = new frmImport(ImportMode.IPlatePlan, hiddenNames))
                    {
                        if (myForm.ShowDialog() == DialogResult.OK)
                        {
                            BindingList<PlatePlan> prdList = (BindingList<PlatePlan>)myForm.objLists;
                            BindingList<PlatePlan> xcList = (BindingList<PlatePlan>)gridControl2.DataSource;
                            foreach (PlatePlan prd in prdList)
                                xcList.Add(prd);
                        }
                    }
                ...
                {
                    using (frmImport myForm = new frmImport(ImportMode.IPartsPlan, hiddenNames))
                    {
                        if (myForm.ShowDialog() == DialogResult.OK)
                        {
                            BindingList<PartsPlan> prdList = (BindingList<PartsPlan>)myForm.objLists;
                            BindingList<PartsPlan> xcList = (BindingList<PartsPlan>)gridControl2.DataSource;
                            foreach (PartsPlan prd in prdList)
                                xcList.Add(prd);
                        }
                    }
