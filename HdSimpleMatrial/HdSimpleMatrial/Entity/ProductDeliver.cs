...
{
    public class ProductDeliver : Product, DbInterface
    {
        /// <summary>
        /// ID
        /// </summary>
        [Category("属性"), DisplayName("源明细ID"), Browsable(false)]
        public long SourceID { get; set; }
        /// <summary>
        /// 源单据编号
        /// </summary>
        [Category("属性"), DisplayName("源单据编号"), Browsable(false)]
        public string SourceSheetNmber { get; set; }



        public bool AddNew()
        {
            string sql = "INSERT INTO ProductDeliver(SheetName,ProcutSeries,WindowsNumber,Color,OpenStyle,Width,Height,"
                + "Number,Area,Price,Location,BarCode,Info,SourceID,SourceSheetNmber) VALUES('"
                ...
                + this.Location + "','" + this.BarCode + "','" + this.Info + "'," + this.SourceID + ",'" + this.SourceSheetNmber + "')";
            try
            {
        ...
        public bool Delete()
        {
            ...
            string sql = "DELETE FROM ProductDeliver WHERE ID = " + this.ID;
            try
            {
                using (ChannelFactory<IhdSQLite> channelFactory = new ChannelFactory<IhdSQLite>("hdDbClient"))
                {
        ...
        {
            if (this.ID <= 0) return false;
            string sql = "UPDATE ProductDeliver SET SheetName='" + this.SheetName + "',ProcutSeries='" + this.ProcutSeries + "',WindowsNumber='" + this.WindowsNumber
                ...
                ",Location='" + this.Location + "',BarCode='" + this.BarCode + "',Info='" + this.Info +
                "',+SourceID=" + this.SourceID + ",SourceSheetNmber='" + this.SourceSheetNmber + "' WHERE ID=" + this.ID;
            try
            {
                //查找用户
                using (ChannelFactory<IhdSQLite> channelFactory = new ChannelFactory<IhdSQLite>("hdDbClient"))
        ...
        {
            string sql = "SELECT * FROM ProductDeliver WHERE ID=" + id;
            try
            {
                using (ChannelFactory<IhdSQLite> channelFactory = new ChannelFactory<IhdSQLite>("hdDbClient"))
                {
                    ...
                    using (OperationContextScope loginScope = new OperationContextScope(myFile as IClientChannel))
                    {
                        ...
                        if (dt.Rows.Count > 0)
                        {
                            ...
                            this.SourceID = Convert.ToInt32(dr["SourceID"]);
                            this.SourceSheetNmber = dr["SourceSheetNmber"].ToString();
                        }
                        else
